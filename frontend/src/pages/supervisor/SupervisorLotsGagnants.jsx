...
import { 
  Trophy, RefreshCw, Search, DollarSign, CheckCircle, Clock, Eye, Printer, Users, Download, Check, X, Calendar, Filter
} from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';


export const SupervisorLotsGagnants = () => {
  const { token, user } = useAuth();
  const [loading, setLoading] = useState(true);
  const [tickets, setTickets] = useState([]);
  const [summary, setSummary] = useState({ total_count: 0, total_win_amount: 0, paid_count: 0, pending_count: 0, by_agent: {} });
  ...
  const fetchWinningTickets = async () => {
    try {
      setLoading(true);
      let url = `${API_URL}/api/supervisor/winning-tickets`;
      const params = [];
      
      if (filterPaymentStatus !== 'all') {
        params.push(`payment_status=${filterPaymentStatus}`);
      }
  ...
  useEffect(() => {
    fetchWinningTickets();
  }, [token, filterPaymentStatus, dateFilter]);
  ...
  const exportToExcel = () => {
    window.open(`${API_URL}/api/export/supervisor/winning-tickets?token=${token}`, '_blank');
  ...
  const updatePaymentStatus = async (ticketId, newStatus) => {
    ...
    try {
      await axios.put(
        `${API_URL}/api/supervisor/winning-tickets/${ticketId}/payment-status`,
  ...
  return (
    <div className="space-y-6" data-testid="supervisor-lots-gagnants">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400">Tickets gagnants de tous vos agents</p>
        </div>
...
export default SupervisorLotsGagnants;
