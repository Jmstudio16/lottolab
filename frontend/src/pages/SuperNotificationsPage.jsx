...
import { 
  ...
  Trophy, DollarSign, Ticket, Users, AlertCircle, Filter, Archive,
...
export const SuperNotificationsPage = () => {
  ...
  const fetchNotifications = useCallback(async () => {
    try {
      ...
      const res = await axios.get(`${API_URL}/api/saas/notifications?limit=200`, { headers });
      const allNotifs = Array.isArray(res.data) ? res.data : (res.data.notifications || []);
      
      // Separate active and archived (read more than 24h ago)
      const now = new Date();
      const active = [];
      const archived = [];
      ...
      allNotifs.forEach(n => {
        ...
        // Archive: read notifications older than 24h
  ...
  return (
    <AdminLayout>
      <div className="p-4 sm:p-6 space-y-6" data-testid="super-notifications-page">
        {/* Header */}
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-white flex items-center gap-3">
              <Bell className="w-6 h-6 sm:w-7 sm:h-7 text-amber-400" />
...
export default SuperNotificationsPage;
