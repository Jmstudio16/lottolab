...
export const CompanyFichesSupprimees = () => {
  const { token } = useAuth();
  ...
  const fetchDeletedTickets = async () => {
    try {
      setLoading(true);
      const res = await axios.get(`${API_URL}/api/company/deleted-tickets`, { headers });
  ...
  return (
    <div className="p-6 space-y-6" data-testid="company-fiches-supprimees">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400">Tous les tickets annulés de votre entreprise</p>
        </div>
...
export default CompanyFichesSupprimees;
