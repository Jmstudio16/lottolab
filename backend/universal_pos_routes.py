...
@universal_pos_router.get("/device/config")
async def get_device_config(current_agent: dict = Depends(get_current_agent)):
    """Get all configuration for POS startup"""
...
# ============ SYNC ENDPOINT ============
@universal_pos_router.get("/device/sync")
async def sync_device(current_agent: dict = Depends(get_current_agent)):
    ...
    daily_stats = stats_result[0] if stats_result else {"total_tickets": 0, "total_sales": 0, "total_wins": 0}
    ...
    return {
        ...
        "daily_stats": {
            "tickets": daily_stats.get("total_tickets", 0),
            "sales": daily_stats.get("total_sales", 0.0),
            "wins": daily_stats.get("total_wins", 0.0),
            "net": daily_stats.get("total_sales", 0.0) - daily_stats.get("total_wins", 0.0)
        },
        "balance": {
...
@universal_pos_router.get("/ticket/print/{ticket_id}", response_class=HTMLResponse)
async def print_ticket(
    ticket_id: str,
    current_agent: dict = Depends(get_current_agent)
):
    """Generate printable ticket HTML for thermal (80mm) and standard printers"""
    agent_id = current_agent.get("user_id")
    company_id = current_agent.get("company_id")
    ...
    # Generate QR code
    qr_code = generate_qr_code_base64(ticket.get("qr_payload", ticket["ticket_code"]))
    ...
    for play in ticket.get("plays", []):
        plays_html += f"""
        <tr>
            <td>{play.get('numbers', '')}</td>
            <td>{play.get('bet_type', '')}</td>
            <td style="text-align: right;">{play.get('amount', 0):.2f}</td>
        </tr>
        """
    
    # Receipt header/footer from config
...
<body>
    ...
    <div class="header">
        ...
        {f'<div>{receipt_header}</div>' if receipt_header else ''}
    </div>
    
    <div class="ticket-info">
        <div><strong>Loterie:</strong> {ticket.get('lottery_name', '')}</div>
        <div><strong>Tirage:</strong> {ticket.get('draw_name', '')} - {ticket.get('draw_date', '')}</div>
        <div><strong>Agent:</strong> {ticket.get('agent_name', '')}</div>
        <div><strong>Appareil:</strong> {ticket.get('device_type', '')}</div>
        <div><strong>Date:</strong> {ticket.get('created_at', '')[:19].replace('T', ' ')}</div>
    </div>
    ...
    <div class="total">
        TOTAL: {ticket.get('total_amount', 0):.2f} {ticket.get('currency', 'HTG')}
    </div>
    
    <div class="total" style="font-size: 12px;">
        Gain Potentiel: {ticket.get('potential_win', 0):.2f} {ticket.get('currency', 'HTG')}
    </div>
    ...
    <div class="verification">
        Code de vérification: {ticket.get('verification_code', '')}
    </div>
    
    <div class="footer">
        {receipt_footer}
        <br>
        Conservez ce ticket pour réclamation
    </div>
    
    <script>
        // Auto-print on load for POS devices
