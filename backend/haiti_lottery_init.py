"""
Haiti Lottery Initialization Script
This script ensures all Haiti lotteries are properly configured with schedules.
Can be run standalone or imported by server.py for automatic initialization.
"""
...
async def initialize_haiti_lotteries(db):
    """
    Initialize all Haiti lotteries with proper schedules.
    This function is idempotent - safe to run multiple times.
    """
    logger.info("[HAITI INIT] Starting Haiti lottery initialization...")
    ...
    for lottery_def in HAITI_LOTTERIES:
        ...
        if not existing_lottery:
            ...
            logger.info(f"[HAITI INIT] Created lottery: {lottery_name}")
            created_count += 1
        else:
    ...
    logger.info(f"[HAITI INIT] Complete! Created: {created_count}, Updated: {updated_count}")
    return {"created": created_count, "updated": updated_count}

async def run_standalone():
    """Run the initialization as a standalone script"""
    ...
    try:
        ...
        print(f"✅ Haiti lottery initialization complete!")
